...
class eliteTorrentProvider(generic.TorrentProvider):
    def __init__(self):
        ...
        self.urls = {
            ...
            'search': 'http://www.elitetorrent.net/busqueda/'
        }
        ...
        """
        Search query:
        http://www.elitetorrent.net/busqueda/heroes+reborn/modo:listado
    ...
    def _doSearch(self, search_strings, search_mode='eponly', epcount=0, age=0, epObj=None):
        ...
        for mode in search_strings.keys():
            ...
            for search_string in search_strings[mode]:
                ...
                search_url = self.urls['search'] + urllib.parse.urlencode(search_string.strip()) + '/modo:listado'
                logger.log(u"Search URL: %s" % search_url, logger.DEBUG)
                data = self.getURL(search_url, post_data=self.search_params, timeout=30)
                if not data:
                    continue
                ...
                try:
                    with BS4Parser(data, features=["html5lib", "permissive"]) as html:
                        ...
                        for row in torrent_table:
                            try:
                                if iteration > 0:
                                    torrent_row = row.findAll('td')[0]
                                    torrent_url = torrent_row.findAll('a')[0]
                                    torrent_name = torrent_row.findAll('span')[0]

                                    download_url = torrent_url.get('href')
                                    title_raw = torrent_name.text
                                    size = -1
